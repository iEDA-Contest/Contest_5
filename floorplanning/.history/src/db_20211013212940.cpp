...
void Module::cutIntoRect(vector<Point*> boundarys)
{
    ...
    case Shape::LSHAPE :
    {
        float left_height  = _boundary[0]->getY() - _boundary[1]->getY();
        float right_height = _boundary[4]->getY() - _boundary[5]->getY();
    ...
    case Shape::TSHAPE :
    {
        ...
        if(_boundary[2]->getX() == _boundary[4]->getX())//T shape cut into two rectangles
        {
            float left_height  = _boundary[0]->getY() - _boundary[1]->getY();
            float right_height = _boundary[6]->getY() - _boundary[7]->getY();

            lx_1 = _boundary[0]->getX();
            ly_1 = _boundary[0]->getY();
            hx_1 = _boundary[2]->getX();
            hy_1 = ly_1 + left_height;
            ...
            lx_2 = _boundary[2]->getX();
            ly_2 = _boundary[7]->getY();
            hx_2 = _boundary[6]->getX();
            hy_2 = ly_2 + right_height;
        ...
        else//T shape cut into three rectangles
        {
            ...
            //set all x
            lx_1 = _boundary[1]->getX();
            hx_1 = lx_2 = _boundary[2]->getX();
            hx_2 = lx_3 = _boundary[4]->getX();
            hx_3 = _boundary[6]->getX();

            //set left and right sub module's y
            ly_1 = _boundary[1]->getY();
            hy_1 = _boundary[0]->getY();
            ly_3 = _boundary[7]->getY();
            hy_3 = _boundary[6]->getY();

            //set mid sub module's y
            if(_boundary[2]->getY() == _boundary[4]->getY())
            {
                ly_2 = ly_1;
                hy_2 = _boundary[2]->getY();
            }
            else if(_boundary[3]->getY() == _boundary[5]->getY())
            {
                ly_2 = _boundary[3]->getY();
                hy_2 = hy_1;
            }
            else if(_boundary[2]->getY() > _boundary[4]->getY())
            {   
                ly_2 = _boundary[1]->getY();
                hy_2 = _boundary[6]->getY();
            }
            else if(_boundary[2]->getY() < _boundary[4]->getY())
            {
                ly_2 = _boundary[7]->getY();
                hy_2 = _boundary[0]->getY();
            }else
            {
                cout << "cut rect error!!!!!!\n";
            }
            ...
            Point* left_low_mid = new Point(lx_1, ly_1);
            sub_module_mid->addBoundaryPoint(left_low_mid);
            Point* right_low_mid = new Point(hx_1, ly_1);
            sub_module_mid->addBoundaryPoint(right_low_mid);
            Point* right_up_mid = new Point(hx_1, hy_1);
            sub_module_mid->addBoundaryPoint(right_up_mid);
            Point* left_up_mid = new Point(lx_1, hy_1);

...
float DataService::getLeftXOnAreaLimit(float high_y, float low_y)
{
    Point* pointNextToRange1;
    Point* pointNextToRange2;
    vector<Point*> pointsInRange;
    size_t size = _area.size();

    pointNextToRange1 = _area[0];
    pointNextToRange2 = _area[1];

    for (uint32_t i = size - 1; i > 0; i--)
    {
        if(_area[i]->getY() <= high_y && _area[i]->getY() >= low_y)
        {
            pointsInRange.push_back(_area[i]);
            pointNextToRange2 = _area[i - 1];
        }else
        {
    ...
    {
        float lx, hx;
        lx  = pointNextToRange1->getX() <= pointNextToRange2->getX() ? pointNextToRange1->getX() : pointNextToRange2->getX();
        hx = pointNextToRange1->getX() > pointNextToRange2->getX() ? pointNextToRange1->getX() : pointNextToRange2->getX();
        if(lx == hx)
        {
            return lx;
        }
        ...
        {
            float ly, hy;
            ly  = pointNextToRange1->getY() <= pointNextToRange2->getY() ? pointNextToRange1->getY() : pointNextToRange2->getY();
            hy = pointNextToRange1->getY() > pointNextToRange2->getY() ? pointNextToRange1->getY() : pointNextToRange2->getY();

            float x_diff = hx - lx;
            float y_diff = hy - ly;
            float x_y_ratio = x_diff / y_diff;

            float left_x_1 = lx + (low_y - ly) * x_y_ratio;
            float left_x_2 = lx + (high_y - ly) * x_y_ratio;
    ...
    {
        size = pointsInRange.size();

        //Calculate the X coordinate of the first candidate
        float left_x_1 = pointsInRange[0]->getX();
        ...
        lx = pointNextToRange1->getX() <= pointsInRange[0]->getX() ? pointNextToRange1->getX() : pointsInRange[0]->getX();
        hx = pointNextToRange1->getX() > pointsInRange[0]->getX() ? pointNextToRange1->getX() : pointsInRange[0]->getX();

        ly = pointNextToRange1->getY() <= pointsInRange[0]->getY() ? pointNextToRange1->getY() : pointsInRange[0]->getY();
        hy = pointNextToRange1->getY() > pointsInRange[0]->getY() ? pointNextToRange1->getY() : pointsInRange[0]->getY();

        float x_diff = hx - lx;
        float y_diff = hy - ly;
        float x_y_ratio = x_diff / y_diff;

        float left_x_2 = lx + (low_y - ly) * x_y_ratio;

        //Calculate the X coordinate of the second candidate
        lx = pointNextToRange2->getX() <= pointsInRange[size - 1]->getX() ? pointNextToRange2->getX() : pointsInRange[size - 1]->getX();
        hx = pointNextToRange2->getX() > pointsInRange[size - 1]->getX() ? pointNextToRange2->getX() : pointsInRange[size - 1]->getX();

        ly = pointNextToRange2->getY() <= pointsInRange[size - 1]->getY() ? pointNextToRange2->getY() : pointsInRange[size - 1]->getY();
        hy = pointNextToRange2->getY() > pointsInRange[size - 1]->getY() ? pointNextToRange2->getY() : pointsInRange[size - 1]->getY();

        x_diff = hx - lx;
        y_diff = hy - ly;
        x_y_ratio = x_diff / y_diff;

        float left_x_3 = lx + (high_y - ly) * x_y_ratio;

...
void Module::cutIntoRect(vector<Point*> boundarys)
{
    ...
    case Shape::LSHAPE :
    {
        ...
        sub_module_left->setWidth();
        sub_module_left->setHeight();
        ...
        sub_module_right->setWidth();
        sub_module_right->setHeight();
    ...
    case Shape::TSHAPE :
    {
        ...
        else//T shape cut into three rectangles
        {
            ...
            sub_module_mid->setWidth();
            sub_module_mid->setHeight();
...
{
    //init _outline_points
    _outline_points.clear();
    _outline_points.push_back(_area[0]);
...
float DataService::getLeftXOnOutlineLimit(float low_y, float high_y)
{
    ...
    vector<Point*> points_in_range;
    size_t size = _outline_points.size();

    for (uint32_t i = 1; i < size - 1; i++)
    {
        if(_outline_points[i]->getY() <= low_y && _outline_points[i + 1]->getY() >= high_y)
        {
...
float DataService::getBottomYOnOutlineLimit(float low_x, float high_x)
{
    ...
    for (uint32_t i = size - 1; i > 0; i--)
    {
        if(_outline_points[i]->getY() <= low_x && _outline_points[i - 1]->getY() >= high_x)
        {
            return _outline_points[i]->getY();
        }
        if(_outline_points[i]->getY() > low_x && _outline_points[i]->getY() < high_x)
        {
            points_in_range.push_back(_outline_points[i]);
        }
    }

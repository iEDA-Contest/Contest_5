...
float DataService::getCoordinateOnAreaLimit(float low, float high)
{
    ...
    {
        if(_area[i]->getY() <= high && _area[i]->getY() >= low)
        {
            pointsInRange.push_back(_area[i]);
            pointNextToRange2 = _area[i - 1];
        }else
        {
            if(_area[(i + 1) % size]->getY() <= high && _area[(i + 1) % size]->getY() >= low)
            {
                break;
            }
    ...
    if(pointsInRange.empty())
    {
        ...
        else
        {
            ...
            float left_x_1 = x1 + (low - y1) * x_y_ratio;
            float left_x_2 = x2 + (high - y2) * x_y_ratio;
    ...
    else
    {
        ...
        float left_x_2 = x1 + (low - y1) * x_y_ratio;

        //Calculate the X coordinate of the second candidate
        x1 = pointsInRange[size - 1]->getX();
        x2 = pointNextToRange2->getX();

        y1 = pointsInRange[size - 1]->getY();
        y2 = pointNextToRange2->getY();

        x_diff = x1 - x2;
        y_diff = y1 - y2;
        x_y_ratio = x_diff / y_diff;

        float left_x_3 = x1 + (high - y1) * x_y_ratio;

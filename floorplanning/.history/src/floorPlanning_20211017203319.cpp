...
float DataService::getLeftXOnAreaLimit(float low_y, float high_y)
{
    ...
    {
        if(_area[(i + 1) % size]->getY() < low_y && _area[i]->getY() >high_y)
        {
    ...
    float low_or_high_y = high_y - y1 < low_y - y1 ? high_y : low_y;

    float left_x_2 = x1 + (low_or_high_y - y1) * x_y_ratio;
    ...
    low_or_high_y = high_y - y1 < low_y - y1 ? high_y : low_y;
...
float DataService::getTopYOnAreaLimit(float low_x, float high_x)
{
    ...
    for (uint32_t i = size - 1 ; i > 0; i--)
    {
        if(_area[(i + 1) % size]->getX() < low_x && _area[i]->getX() >high_x)
        {
            float x1 = _area[(i + 1) % size]->getX();
            float x2 = _area[i]->getX();
            float y1 = _area[(i + 1) % size]->getY();
        ...
        else
        {
            ...
            {
                if(_area[(i + 1) % size]->getX() < high_x && _area[(i + 1) % size]->getX() > low_x)
                {
                    break;
                }
                last_point_of_range = _area[i];
    ...
    float low_or_high_x = high_x - x1 < low_x - x1 ? high_x : low_x;

    float top_y_2 = y1 + (low_or_high_x - x1) * y_x_ratio;
    ...
    low_or_high_x = high_x - x1 < low_x - x1 ? high_x : low_x;

    float top_y_3 = y1 + (low_or_high_x - x1) * y_x_ratio;

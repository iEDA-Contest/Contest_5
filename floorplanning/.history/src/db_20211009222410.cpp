...
{
    for (size_t i = 0; i < _boundary.size(); i++)
    {
        if(_boundary[i + 1]->getY() < _boundary[i]->getY())
        {
            Point* temp     = _boundary[i];
            _boundary[i]     = temp;
            _boundary[i + 1] = temp;
        }
    }
    for (size_t i = 0; i < _boundary.size(); i++)
    {
        if(_boundary[i + 1]->getX() < _boundary[i]->getX())
        {
            Point* temp     = _boundary[i];
            _boundary[i]     = temp;
            _boundary[i + 1] = temp;
...
void Module::cutIntoRect()
{
    ...
    case Shape::LSHAPE :
    {
        ...
        Point* left_up = new Point(lx_1, hy_1);
        sub_module_left->addBoundaryPoint(left_up);
        Point* left_low = new Point(lx_1, ly_1);
        sub_module_left->addBoundaryPoint(left_low);
        Point* right_low = new Point(hx_1, ly_1);
        sub_module_left->addBoundaryPoint(right_low);
        Point* right_up = new Point(hx_1, hy_1);
        sub_module_left->addBoundaryPoint(right_up);
        ...
        Point* left_up = new Point(lx_1, hy_1);
        sub_module_right->addBoundaryPoint(left_up);
        Point* left_low = new Point(lx_1, ly_1);
        sub_module_right->addBoundaryPoint(left_low);
        Point* right_low = new Point(hx_1, ly_1);
        sub_module_right->addBoundaryPoint(right_low);
        Point* right_up = new Point(hx_1, hy_1);
        sub_module_right->addBoundaryPoint(right_up);

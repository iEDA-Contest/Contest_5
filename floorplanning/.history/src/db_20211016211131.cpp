...
float DataService::getLeftXOnAreaLimit(float high_y, float low_y)
{
    ...
    {
        float left, right;
        left  = pointNextToRange1->getX() <= pointNextToRange2->getX() ? pointNextToRange1->getX() : pointNextToRange2->getX();
        right = pointNextToRange1->getX() > pointNextToRange2->getX() ? pointNextToRange1->getX() : pointNextToRange2->getX();
        if(left == right)
        {
            return left;
        }else
